...
const emit = defineEmits(["new_item"]);
...
const create = async () => {
    try {
        if (v$.value.$invalid) {
            ...
            let response = await axios.post("/api/products/create-product", form);
            const { message, status, data } = response.data;
            message_alert.value = message;
            status_alert.value = status;
            if (status == 201) {
                emit("new_item", data[0]);
                clean();
            } else {
                loading.value = false;
            }
...
<template>
    <v-dialog v-model="dialog" persistent>
        <template v-slot:activator="{ props }">
            <v-btn :prepend-icon="mdiPlusCircleOutline" v-bind="props" color="success">
                Create Product
            </v-btn>
        </template>
        <v-card>
            <form @submit.prevent="create()">
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="delete" variant="flat" size="small" @click="
                        clean(),
                        (dialog = false),
                        (message_alert = null),
                        (loading = false)
